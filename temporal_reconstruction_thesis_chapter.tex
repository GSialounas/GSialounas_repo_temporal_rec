...
\begin{Defn}[AlternativePolynomial version of the reconstruction from \cite{georgoulis2016posteriori}]\label{defn_rec_paper_poly+tristan}
	The polynomial version of the reconstructions in Defn. \ref{defn_rec_paper} are given by 
	\begin{equation}\label{eq_rec_paper_poly_alternative}
	\begin{aligned}
	\hat{U}|_{\qb{t^n{n-1} t^{n}}}\qp{t}&=c_{0,u}+c_{1,u}\qp{t-t^{n-1}}+c_{2,u}\qp{t-t^{n-1}}^2\quad \text{and}\\
	\hat{V}|_{\qb{t^{n-1/2}, t^{n+1/2}}}\qp{t}&=c_{0,v}+c_{1,v}\qp{t-t^{n-1/2}}+c_{2,v}\qp{t-t^{n-1/2}}^2\quad\text{in }\qb{t_{n-1/2},t_{n+1/2}}.
	\end{aligned}
	\end{equation}
	where
	...
	\begin{aligned}
	c_{0,u} &= U^{n},\\
	c_{1,u}&= (V^{n-1}+\rho_V)=-\frac{1}{4}V^{n+1/2}+V^{n-1/2}+\frac{1}{4}V^{n-3/2},\\
	c_{2,u}&=\frac{1}{4\Delta t}\qp{V^{n+1/2}-V^n-2/},\\
	c_{0,v} &= V^{n-1/2},\\
