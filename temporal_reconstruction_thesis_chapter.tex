...
&\rec{U}'|_{\qpb{t^n,t^{n+1}}}\qp{t^{n+1}}=V_1\qp{t^{n+1}}+\rho_V\\
&\rec{V}|_{\qb{t^{n-1/2},t^{n+1/2}}}\qp{t^i}=V^i \quad \text{for}\quad i= n-1/2, n+1/2,\\
&\rec{V}'|_{\qpb{t^{i-1/2},t^{i+1/2}}}\qp{t^{n+1/2}}=-U_1\qp{t^{n+1/2}}+\rho_U,
\end{aligned}
	\end{equation}
	and they lead to the coefficients given below
	\begin{equation}
	\begin{aligned}
	c_{0,u} &= U^{n},\\
	...
	c_{2,v}&=\frac{1}{4\Delta t}\qp{U^{n+1}-U^{n-1}},\\
	\end{aligned}
	\end{equation}
\end{Defn}

\begin{Defn}[ ]\label{defn_our_rec3}
	The reconstruction, $\qp{\rec{U},\rec{V}}$ of the numerical solution $\qp{U,V}$ of (\ref{eq_harmonic_discretization}) ona staggered grid consists of the unique functions $\rec{U}\in \mathbb{V}_2$, $\rec{V}\in\mathbb{V}_3$ satisfying
	\begin{equation}\label{eq_rec3_conds}
	\begin{aligned}
	&\rec{U}|_{\qb{t^n,t^{n+1}}}\qp{t^i}=U^i \quad \text{for}\quad i= n, n+1,\\
	&\rec{U}''|_{\qb{t^n,t^{n+1}}}\qp{t^n}=-U^n,\\
	&\rec{V}|_{\qb{t^{n-1/2},t^{n+1/2}}}\qp{t^i}=V^i \quad \text{for}\quad i= n-1/2, n+1/2,\\
	&\rec{V}'|_{\qb{t^{i-1/2},t^{i+1/2}}}\qp{t^i}=-\rec{U}|_{\qb{t^i,t^{i+1}}}\qp{t^i},\quad i = n, \, n+1\\
	\end{aligned}
	\end{equation}
\end{Defn}
